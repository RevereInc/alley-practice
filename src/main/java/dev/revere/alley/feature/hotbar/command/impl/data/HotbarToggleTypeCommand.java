 ...
 * @since 02/11/2025
 */
public class HotbarToggleTypeCommand extends BaseCommand {
    @CommandData(
            name = "hotbar.toggletype",
            isAdminOnly = true,
            usage = "hotbar toggletype <name> <type>",
            description = "Toggle the type of a hotbar item."
    )
    @Override
    public void onCommand(CommandArgs command) {
        ...
        typeData.setEnabled(!typeData.isEnabled());
        hotbarService.saveToConfig(hotbarItem);
        player.sendMessage(this.getString(GlobalMessagesLocaleImpl.HOTBAR_TOGGLED_TYPE)
                .replace("{hotbar-name}", name)
                .replace("{hotbar-type}", type.name())
                .replace("{status}", typeData.isEnabled() ? "enabled" : "disabled")
        );
    }
}