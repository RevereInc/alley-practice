...
public class KitSetDescriptionCommand extends BaseCommand {
    ...
    public void onCommand(CommandArgs command) {
        ...
        if (args[1].equalsIgnoreCase("clear")) {
            ...
            sender.sendMessage(CC.translate(this.getMessage(GlobalMessagesLocaleImpl.KIT_DESCRIPTION_CLEARED).replace("{kit-name}", kit.getName())));
            return;
        }
        ...
        this.plugin.getService(KitService.class).saveKit(kit);
        sender.sendMessage(CC.translate(this.getMessage(GlobalMessagesLocaleImpl.KIT_DESCRIPTION_SET).replace("{kit-name}", kit.getName()).replace("{description}", description)));
    }
}