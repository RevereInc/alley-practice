...
public class MatchCampProtectionTask extends BukkitRunnable {
    ...
    public void run() {
        ...
        if (ticks <= damageStartPeriod) {
            ...
            String header = localeService.getMessage(VisualLocaleImpl.TITLE_CAMP_PROTECTION_HEADER);
            String footer = localeService.getMessage(VisualLocaleImpl.TITLE_CAMP_PROTECTION_FOOTER).replace("{seconds}", String.valueOf(countdownValue));

            int fadeIn = localeService.getInt(VisualLocaleImpl.TITLE_CAMP_PROTECTION_FADE_IN);
            int stay = localeService.getInt(VisualLocaleImpl.TITLE_CAMP_PROTECTION_STAY);
            int fadeOut = localeService.getInt(VisualLocaleImpl.TITLE_CAMP_PROTECTION_FADEOUT);

            titleReflectionService.sendTitle(this.player, header, footer, fadeIn, stay, fadeOut);
        } else {
            this.player.damage(4.0);

            String header = localeService.getMessage(VisualLocaleImpl.TITLE_CAMP_PROTECTION_TAKING_DAMAGE_HEADER);
            String footer = localeService.getMessage(VisualLocaleImpl.TITLE_CAMP_PROTECTION_TAKING_DAMAGE_FOOTER);

            int fadeIn = localeService.getInt(VisualLocaleImpl.TITLE_CAMP_PROTECTION_TAKING_DAMAGE_FADE_IN);
            int stay = localeService.getInt(VisualLocaleImpl.TITLE_CAMP_PROTECTION_TAKING_DAMAGE_STAY);
            int fadeOut = localeService.getInt(VisualLocaleImpl.TITLE_CAMP_PROTECTION_TAKING_DAMAGE_FADEOUT);
